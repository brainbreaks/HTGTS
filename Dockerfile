...
RUN apt-get update
RUN apt-get install -y bioperl libbio-samtools-perl libtext-csv-perl libfile-which-perl libipc-system-simple-perl ea-utils seqprep bowtie2 python3-tqdm python3-pybedtools python3-pandas

RUN install2.r --error --deps TRUE argparser plyr
RUN /usr/local/lib/R/site-library/littler/examples/installBioc.r GenomicRanges BSgenome

# HTGTS v2
# RUN cd $DESTINATION && git clone https://github.com/robinmeyers/transloc_pipeline.git HTGTS

RUN cpan install Interpolation Storable Switch::Plain
RUN cd /bin && \
    git clone https://github.com/brwnj/fastq-multx.git fastq-multx_source && \
